...
<c:set var="FOLLOW_EACH_ICON"
	value="/weibo/resources/img/follow_each.png" />
<c:set var="FOLLOWED_TEXT" value="已关注" />
<c:set var="UNFOLLOWED_TEXT" value="尚未关注" />
<c:set var="FOLLOWED_EACH_TEXT" value="互相关注" />
...
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/resources/css/concernedUserList.css">
</head>
<body>
	...
	<div class="container">
		...
		<div class="content">
			<ul class="list-group">
					...
					<h4 class="userName">${relativeUser.user.nickname}<img
						...
						<form class="buttonForm">
							<div class="followCancer">
								<img
									src="${relativeUser.isMyFan==0?FOLLOW_ICON:FOLLOW_EACH_ICON}"
									class="follow_icon"><span class="allFollow_text">${relativeUser.isMyFan==0?FOLLOWED_TEXT:FOLLOWED_EACH_TEXT}</span>
							</div>
							<input type="button"
				...
				<c:if
					test="${fn:length(pagerResult.resultSet)==0||pagerResult.totalPage<=1}">
	...
	<script
		src="<%=request.getContextPath()%>/resources/interactive/personal/concernedUserList.js"></script>
	<script
